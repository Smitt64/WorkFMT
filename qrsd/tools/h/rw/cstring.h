 ...
 *
 * $Id: cstring.h@#/main/64  09/03/97 18:09:16  kevinj (TOOLS707_WIN32_971021)
 *
...
# endif
  ...
  RWspace       binaryStoreSize() const         {return (RWspace)(length() + sizeof(size_t));}
...
# endif
  ...
  RW_SL_IO_STD(istream&)     readFile(RW_SL_IO_STD(istream&) is);                     // Read to EOF or null character.
  RW_SL_IO_STD(istream&)     readLine(RW_SL_IO_STD(istream&) is, RWBoolean skipWhite = TRUE);   // Read to EOF or newline.
  RW_SL_IO_STD(istream&)     readString(RW_SL_IO_STD(istream&) is);                   // Read to EOF or null character.
  RW_SL_IO_STD(istream&)     readToDelim(RW_SL_IO_STD(istream&) is, char delim='\n'); // Read to EOF or delimitor.
  RW_SL_IO_STD(istream&)     readToken(RW_SL_IO_STD(istream&) is);                    // Read separated by white space.
...
{ return replace(length(), 0, s.data(), rwmin((unsigned int)N, (unsigned int)s.length())); }
...
{ return replace(pos, 0, cstr.data(), rwmin((unsigned int)N, (unsigned int)cstr.length())); }
...
{ return replace(0, 0, s.data(), rwmin((unsigned int)N, (unsigned int)s.length())); }
...
{ return replace(pos, n1, cstr.data(), rwmin((unsigned int)cstr.length(), (unsigned int)n2)); }
